...
def get_local_explorer_folder_under_cursor() -> str | None:
    """
    마우스 커서가 가리키는 최상위 윈도우가 Explorer면 해당 폴더 경로 반환.
    (뷰어 창이 포커스를 갖고 있어도 동작)
    """
    try:
...
class ControlClient:
    ...
    def connect(self):
        ...
        try:
            ...
            self.sock.settimeout(3.0); self.sock.connect((self.host,self.port)); self.sock.settimeout(None)
        except Exception:
            self.sock=None
    def send_json(self, obj:dict):
        ...
        try:
            body=json.dumps(obj).encode("utf-8"); head=struct.pack(">I",len(body))
